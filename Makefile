...
VECT_SRC		= src/vector/vector_test.hpp src/vector/assign().cpp src/vector/assign(std_iterators).cpp src/vector/at().cpp src/vector/back().cpp src/vector/begin().cpp src/vector/capacity().cpp src/vector/clear().cpp src/vector/constructor.cpp src/vector/constructor(std_iterators).cpp src/vector/data().cpp src/vector/empty().cpp src/vector/end().cpp src/vector/erase(range).cpp src/vector/erase(value).cpp src/vector/front().cpp src/vector/insert(fill).cpp src/vector/insert(range).cpp src/vector/insert(std_iterators).cpp src/vector/insert(value).cpp src/vector/iterators.cpp src/vector/max_size().cpp src/vector/operator!=.cpp src/vector/operator<.cpp src/vector/operator<=.cpp src/vector/operator==.cpp src/vector/operator>.cpp src/vector/operator>=.cpp src/vector/operator[].cpp src/vector/pop_back().cpp src/vector/push_back().cpp src/vector/rbegin().cpp src/vector/rend().cpp src/vector/reserve().cpp src/vector/resize().cpp src/vector/size().cpp src/vector/swap().cpp src/vector/using_allocator.cpp
...
STACK_SRC		= src/stack/stack_test.hpp src/stack/constructor.cpp src/stack/empty().cpp src/stack/operator!=.cpp src/stack/operator<.cpp src/stack/operator<=.cpp src/stack/operator==.cpp src/stack/operator>.cpp src/stack/operator>=.cpp src/stack/pop().cpp src/stack/push().cpp src/stack/size().cpp src/stack/top().cpp
...
$(NAME_STD):	$(HEADER) $(OBJS_STD) $(VECT_OBJS) $(MAP_OBJS) $(STACK_OBJS) $(UTILITY_OBJS)
			$(CPP) $(CPPFLAGS) -o $(NAME) $(OBJS_STD) $(VECT_OBJS) $(MAP_OBJS) $(STACK_OBJS) $(UTILITY_OBJS)
		
$(NAME_FT):	$(HEADER) $(OBJS_FT) $(VECT_OBJS) $(MAP_OBJS) $(STACK_OBJS) $(UTILITY_OBJS)
